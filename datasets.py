...
# Fix numpy._core compatibility issue for pickle files created with different numpy versions
...
class S2T_Dataset(Base_Dataset):
    def __init__(self, path, args, phase):
        ...
        elif "OSL-Words" == self.args.dataset:
            self.pose_dir = os.path.join(pose_dirs[args.dataset], phase, "pose")
            self.rgb_dir = os.path.join(rgb_dirs[args.dataset], phase, "rgb")
