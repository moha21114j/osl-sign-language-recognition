...
def main(args):
    ...
    if args.finetune != '':
        ...
        # Filter out size-mismatched keys for SwiGLU compatibility
        model_state = model.state_dict()
        filtered = {k: v for k, v in state_dict.items()
                    if k in model_state and v.shape == model_state[k].shape}
        skipped = [k for k in state_dict if k not in filtered]
        if skipped:
            print(f'Skipped {len(skipped)} mismatched keys (SwiGLU arch change)')
    ...
    lr_scheduler = get_scheduler(
                ...
                num_training_steps=int(max(1, (args.epochs - args.start_epoch)) * len(train_dataloader)/args.gradient_accumulation_steps),
            )
    
    model, optimizer, lr_scheduler = utils.init_deepspeed(args, model, optimizer, lr_scheduler)
    
    # Handle both DeepSpeed wrapped and non-wrapped models
    ...
    use_deepspeed = hasattr(model, 'bfloat16_enabled')

    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            train_sampler.set_epoch(epoch)
        
        # Pass lr_scheduler for non-DeepSpeed training
        train_stats = train_one_epoch(args, model, train_dataloader, optimizer, epoch, 
                                       lr_scheduler=None if use_deepspeed else lr_scheduler)

        # per-epoch checkpoint saving disabled — only best dev and best test are saved
            ...
            if args.task == "SLT":
                ...
                if max_accuracy < test_stats["bleu4"]:
                    ...
                    if args.output_dir and utils.is_main_process():
                        ...
                        }, output_dir / 'best_dev_checkpoint.pth')
                        print(f"  --> New best dev BLEU-4: {max_accuracy:.4f}% - saved best_dev_checkpoint.pth")

                # Evaluate on test and save best test checkpoint
                test_eval_stats = evaluate(args, test_dataloader, model, model_without_ddp, phase='test')
                if max_test_accuracy < test_eval_stats["bleu4"]:
                    max_test_accuracy = test_eval_stats["bleu4"]
                    if args.output_dir and utils.is_main_process():
                        utils.save_on_master({
                            'model': get_requires_grad_dict(model_without_ddp),
                        }, output_dir / 'best_test_checkpoint.pth')
                        print(f"  --> New best test BLEU-4: {max_test_accuracy:.4f}% - saved best_test_checkpoint.pth")

                print(f"[Epoch {epoch}] DEV  - BLEU-1: {test_stats.get('bleu1',0):.4f}  BLEU-3: {test_stats.get('bleu3',0):.4f}  BLEU-4: {test_stats.get('bleu4',0):.4f}  ROUGE-L: {test_stats.get('rouge',0):.4f}")
                print(f"[Epoch {epoch}] TEST - BLEU-1: {test_eval_stats.get('bleu1',0):.4f}  BLEU-3: {test_eval_stats.get('bleu3',0):.4f}  BLEU-4: {test_eval_stats.get('bleu4',0):.4f}  ROUGE-L: {test_eval_stats.get('rouge',0):.4f}")
                print(f"Max dev BLEU-4: {max_accuracy:.4f}% | Max test BLEU-4: {max_test_accuracy:.4f}%")
...
def train_one_epoch(args, model, data_loader, optimizer, epoch, lr_scheduler=None):
    ...
    # Check if using DeepSpeed (has bfloat16_enabled method)
    use_deepspeed = hasattr(model, 'bfloat16_enabled')
    target_dtype = None
    ...
    for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        if target_dtype != None:
            ...
            # Standard PyTorch: move tensors to GPU and ensure float32 dtype
            for key in src_input.keys():
                if isinstance(src_input[key], torch.Tensor):
                    if src_input[key].dtype == torch.float64:
                        src_input[key] = src_input[key].float().cuda()
                    else:
        ...
        # Handle both DeepSpeed and standard PyTorch training
        if use_deepspeed:
