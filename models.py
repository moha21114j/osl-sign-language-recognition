...
class Uni_Sign(nn.Module):
    def __init__(self, args):
        ...
        if self.args.rgb_support:
            ...
            # Change only the LAST Conv1d bias so gate starts at ~0.5
            # The last Conv1d is followed by Tanh then ReLU, so bias=0.5 → output≈0.46
            for layer in reversed(list(self.fusion_gate)):
                if isinstance(layer, nn.Conv1d):
                    nn.init.constant_(layer.bias, 0.5)  # ← Start with meaningful RGB contribution
                    break
